...
module Server.Hardware.Telnet where
...
instance FromNoun TelnetRequest where
    fromNoun = \case
        NAT 0 -> pure OPEN
...
createHardwareTelnet :: Debug => FilePath -> Acquire HardwareFun
createHardwareTelnet machineDir = do
    tt <- mkAcquire (startAll machineDir) shutdown
    pure \m v k -> writeTBQueue tt.inpQ (RR m v k)

-- Shut Down -------------------------------------------------------------------

shutdown :: TelnetState -> IO ()
shutdown st = do
    cogs <- atomically (readTVar st.cogs)
    cancel st.inpT
    for_ cogs \cog -> do
        catchIOError (removeFile cog.portsFile) (const $ pure ())
...
startCog machineDir cogName = do
    ...
    debugTextVal (cogName.txt <> "_telnet_port") (tshow listenPort)
...
startAll :: Debug => FilePath -> IO TelnetState
startAll mach = do
...
acceptConnections sok cog = forever do
    ...
    case mHan of
        Nothing -> do
            N.close socket
            debugText "Telnet Connection Rejected because no WAIT request"
        Just (_key, hSt) -> do
...
--
getCog :: Debug => TelnetState -> CogName -> IO CogState
...
routeRequest :: Debug => TelnetState -> RawRequest -> IO ()
