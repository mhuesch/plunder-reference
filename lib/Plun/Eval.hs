...
import Plun.Print       (decodeBtc, greenOut)
...
valName :: Fan -> Text
valName = \case
    FUN L{..} -> ugul (lawNameNat lawName)
    PIN P{..} -> valName pinItem
    _         -> "_"
 where
    ok '_' = True
    ok c   = C.isAlphaNum c
...
matchPin n (KLO _ e) =
    case (n, GHC.toList e) of
        (0, [NAT 2, x])               -> Just x
        (0, _)                        -> Nothing
        (_, [NAT 0, x, NAT m]) | n==m -> matchPin (n-1) x
        (_, _)                        -> Nothing
matchPin _ _ =
    Nothing
...
tailOptimize = go
  where
    ...
    trkHash   = decodeBtc "DCwrK91sjJddiSyrP7ZMy6Lyu6rpf2tYqf7eqsLcfMhs"
    go = \case
        exe@(EXE _ _ (PIN p) r) ->
            if | pinHash p == ifHash    -> IF_ (go $ r^0) (go $ r^1) (go $ r^2)
               | pinHash p == ifNotHash -> IF_ (go $ r^0) (go $ r^2) (go $ r^1)
               | pinHash p == seqHash   -> SEQ (go $ r^0) (go $ r^1)
               | pinHash p == trkHash   -> TRK (go $ r^0) (go $ r^1)
               | otherwise              -> exe
        LET i v b            -> LET i v (go b)
        exe                  -> exe

fullOptimize :: Run -> Run
fullOptimize = go
  where
    go = \case
        ...
        EXE _ _ COW{} r     -> go $ MK_ROW $ reverse $ fromList $ GHC.toList r
        EXE x s (KLO n e) r ->
            case e^0 of
                COW{} ->
                    go $ MK_ROW
...
executeLaw self pro@PROG{prgrm,stkSz} args =
  ...
  where
    ...
    go vs = \case
        ...
        MK_ROW es -> ROW <$> traverse (go vs) es
